...
class Footer extends Component {
    ...
    if (actualIndex !== 0) {
      return (
        <div className='buttonPannel'>
          <button
            className = { 'buttonNav ' + (actualIndex === 1 || actualIndex === 12 ? 'disabled': '') }
            id = 'btnNavLeft'
  ...
  showPage = () => {
    ...
    if (actualIndex !== 0) {
      return(
        <h2 className = 'numberPage fw-7 azul'>{ ((actualIndex) <= 9 ? '0' + (actualIndex) : (actualIndex)) }/13</h2>
      );
    }
  }
  ...
  render() {
    ...
    return (
      <div className = { (actualIndex === 0 || actualIndex === 13 ? 'footer-desc' : 'footer') + ' d-Flex j-S aI-C' }>

...
class ItemInteractiveAudio extends Component {
  ...
  makeItems = () => {
    const ITEM = this.props.dataItem.map( (item, i) => {
      return(
        <div
          id = { item.time }
          className = { 'itemList c-10 d-Flex d-Rr j-E aI-C pL-1 animated fadeIn '}
          key = { i }
          style = { {} }>
            <p className = 'textList fw-1' dangerouslySetInnerHTML = {{ __html: item.label }}></p>
            <FontAwesomeIcon
            className = 'iconList'
            icon = { ['fas', 'circle'] }
            size = '2x'
  ...
  render() {
    return (
      <div className = 'd-Flex d-C j-S aI-S c-55 wW mB-3 ' style = {{ 'height': 240 }}>
        { this.makeItems() }
      </div>
    );
  }
